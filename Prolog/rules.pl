...
putBuilding(X,Y) :- assert(groundType(X,Y)) .
putAmmo(X,Y) :-  not(mart(X,Y)) , assert(mart(X,Y)) .
putVortex(X,Y) :- not(pokeCenter(X,Y)) , assert(pokeCenter(X,Y)).
putCockroach(X,Y) :- not(trainer(X,Y)) , assert(trainer(X,Y)) , safeLst(L) , takeList(X,Y,L,L1).
putHole(X,Y) :- not(pokeCenter(X,Y)) , assert(pokeCenter(X,Y)).
putBug(X,Y) :- not(pokeCenter(X,Y)) , assert(pokeCenter(X,Y)).
